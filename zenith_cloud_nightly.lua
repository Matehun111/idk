...
client.set_event_callback("paint", function()
    if not menu_enable:get() then return end
    local mode = menu_mode:get()
...
local function LogVerbose(msg)
    if menu_log_verbose:get() then client.color_log(255, 150, 255, "[Zenith] " .. msg .. "\n\0") end
end
client.set_event_callback("aim_hit", function(e)
    if not menu_enable:get() then return end
    local ent = e.target
    ...
    if menu_mode:get() == "Hybrid Engine" and data.last_input then
