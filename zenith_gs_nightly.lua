-- ======================================================================
--  ZENITH | NIGHTLY | Cloud Loader  v2.0.0
...
-- ── HELPERS ───────────────────────────────────────────────────────────
local function clog(r,g,b,m) client.color_log(r,g,b,"[zenith.gs NIGHTLY] "..tostring(m)) end
...
local function get_hwid()
    ...
    local raw   = "ZNHWID:"..tostring(steam)..":nightly:v"..AUTH_VER
    local h = 5381
...
local function load_cloud()
    ...
    http.get(CLOUD_URL, function(success, response)
        ...
        info("zenith.gs NIGHTLY running!")
        -- Hide loader UI
...
local function do_register(key, name, pw)
    ...
    if not VALID_KEYS[key] then
        set_status(255,60,60,"Invalid key. Check prefix: ZNNGT-XXXX-XXXX"); return
    end
...
grp_fl:label('\f<dot>\ac8c8c8ffNIGHTLY  —  Authentication'):depend(_vis)
...
local fl_tier   = grp_fl:label('\f<dot>Tier:   \ac8c8c8ffNIGHTLY'):depend(_vis)
grp_fl:label(' '):depend(_vis)
...
grp_oth:label('\f<dot>\ac8c8c8ff   (ZNNGT-XXXX-XXXX)'):depend(_vis)
