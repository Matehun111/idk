...
LPH_NO_VIRTUALIZE(function ()
    ...
    do
        ...
        -- ── chip bar helpers ────────────────────────────────────────────
        local CHIP_H      = 17    -- chip height px
        local CHIP_PAD_X  = 8     -- horizontal inner padding
        local CHIP_GAP    = 4     -- gap between chips
        local CHIP_R      = 8     -- corner radius (pill shape)
        -- colours (match HTML: #181c23 bg, #2a2f3a border, #8a93a6 text)
        local C_BG        = { 22,  24,  30  }   -- chip bg
        local C_BG_ACT    = { 24,  28,  38  }   -- active chip bg (username)
        local C_BORDER    = { 38,  44,  54  }   -- chip border
        local C_TEXT_DIM  = { 120, 130, 148 }   -- dim text
        local C_TEXT_NORM = { 175, 183, 198 }   -- normal text
        local C_LABEL_BG  = { 16,  18,  24  }   -- "zenith" label bg
        local C_LABEL_BDR = { 33,  38,  48  }   -- "zenith" label border
        ...
        local function draw_chip_rect(x, y, w, h, bg, border, a)
            ...
            renderer.circle(x+w-r2, y+h-r2, bg[1],bg[2],bg[3], ia, r2,   0, 0.25)
            renderer.circle(x+w-r2, y+r2,   bg[1],bg[2],bg[3], ia, r2,  90, 0.25)
            -- border (1px outline via 4 thin rects)
            renderer.rectangle(x+r2,   y,         w-r2*2, 1,   border[1],border[2],border[3], math.floor(ia*0.9))
            renderer.rectangle(x+r2,   y+h-1,     w-r2*2, 1,   border[1],border[2],border[3], math.floor(ia*0.9))
            renderer.rectangle(x,      y+r2,       1,   h-r2*2, border[1],border[2],border[3], math.floor(ia*0.9))
            renderer.rectangle(x+w-1,  y+r2,       1,   h-r2*2, border[1],border[2],border[3], math.floor(ia*0.9))
        end
        ...
        function watermark.frame()
            ...
            -- ── build chip list ──────────────────────────────────────────
            -- each chip: { text, bg, text_col, is_accent }
            local chips = {}

            -- "zenith" italic label (leftmost)
            chips[#chips+1] = { text = "zenith", bg = C_LABEL_BG, bdr = C_LABEL_BDR,
                                 tc = { 100, 108, 122 }, accent = false }

            -- username chip (active style + accent dot)
            if widgets.display:have_key("Username") then
                ...
                chips[#chips+1] = { text = nick, bg = C_BG_ACT, bdr = C_BORDER,
                                     tc = C_TEXT_NORM, accent = true,
                                     ar = ar, ag = ag, ab = ab }
            end
            ...
            if widgets.display:have_key("Latency") then
                ...
                if ping then
                    chips[#chips+1] = { text = ping, bg = C_BG, bdr = C_BORDER, tc = C_TEXT_DIM }
                end
            end
            ...
            -- server frametime (smaller, only if enabled)
            ...
            -- ── position: top-right, 9px from edge ──────────────────────
            ...
            for _, chip in ipairs(chips) do
                ...
                -- accent dot for username chip
                if chip.accent then
                    local dot_r = 3
                    local dot_x = draw_x + CHIP_PAD_X + dot_r
                    local dot_y = cy + math.floor(CHIP_H / 2)
                    renderer.circle(dot_x, dot_y, chip.ar, chip.ag, chip.ab,
                        math.floor(ia * 0.9), dot_r, 0, 1.0)
                    -- text shifted right past dot
                    local tw = renderer.measure_text(flags, chip.text)
                    local tx = draw_x + CHIP_PAD_X + dot_r*2 + 4
                    local ty = cy + math.floor((CHIP_H - select(2, renderer.measure_text(flags, chip.text))) * 0.5)
