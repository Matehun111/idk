...
local TIER       = "nightly"
local AUTH_VER   = 1
local DB_KEYS    = "zenith_nightly_keys_v1"
local DB_SESSION = "zenith_nightly_session_v1"
...
local function clog(r,g,b,msg) client.color_log(r,g,b,"[Zenith NIGHTLY] "..tostring(msg)) end
local function info(msg) clog(200,100,255,msg) end
...
local function validate_key(key)
    ...
    if not entry then
        err("Invalid key.")
        status_msg = "Invalid key. Must start with ZNNGT-"
        return false
    end
    local hwid = get_hwid()
    ...
    auth_ok  = true
    auth_key = key
    status_msg = "Authenticated! Fetching NIGHTLY script..."
    info("Login OK: "..key)
    return true
end
...
local function load_cloud()
    ...
    http.get(CLOUD_URL, function(success, response)
        ...
        -- Use load() with the body string directly (LuaJIT standard)
        ...
        status_msg = "Zenith NIGHTLY loaded!"
...
local function try_restore()
    ...
    auth_ok  = true
    auth_key = sess.key
    status_msg = "Session restored - loading NIGHTLY..."
...
local _t = flbl("NIGHTLY Loader v"..LOADER_VER)
local _s = flbl("Key: ZNNGT-XXXX-XXXX")
local _i1 = flbl("Includes: AA + Aimbot + Resolver")
local _i2 = flbl("5 slots - exclusive tier")
-- Main column
local _h  = lbl("AA", "Zenith NIGHTLY - Authentication")
...
if try_restore() then
    ...
    info("Not authenticated. Enter key ZNNGT-XXXX-XXXX")
    status_msg = "Enter your NIGHTLY key and press Login."
end
