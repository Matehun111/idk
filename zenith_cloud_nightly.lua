    ...
    function defensive.handle(cmd, ctx)
        ...
        do
            ...
            elseif val == 'Snap' then
                -- true random snap: new angle every choke window
                local tc = globals.tickcount()
...
do
    ...
    local _ks_default = {
        ...
        'get rekt | discord.gg/4qSQtPKas5', 'using zenith lol',
    }
...
local NN3 = {
    cfg  = { inp=24, hid=64, out=4, lr=0.032, mom=0.90, dropout=0.15 },
    wh={}, wo={}, vh={}, vo={}, mem={},
    trained_samples = 0,
    best_loss = 1e9,
    lr_schedule = { [50]=0.028, [150]=0.022, [300]=0.016, [500]=0.012 },
}
local function _nn3_xavier(a,b) return (math.random()*2-1)*math.sqrt(6/(a+b)) end
for i=1,24 do NN3.wh[i]={}; NN3.vh[i]={}
    for j=1,64 do NN3.wh[i][j]=_nn3_xavier(24,64); NN3.vh[i][j]=0 end
end
for i=1,64 do NN3.wo[i]={}; NN3.vo[i]={}
    for j=1,4 do NN3.wo[i][j]=_nn3_xavier(64,4); NN3.vo[i][j]=0 end
end

--  ZENITH RESOLVER  v4  (nightly only) - improved classifier, jitter, LBY, brute
...
_M.ui_nn_rst  = menu.new_item(ui.new_button,G,GR,'Reset NN Weights',function()
    local NN=_M.NN3
    for i=1,24 do for j=1,64 do NN.wh[i][j]=(math.random()*2-1)*math.sqrt(6/88) end end
    for i=1,64 do for j=1,4  do NN.wo[i][j]=(math.random()*2-1)*math.sqrt(6/68) end end
    NN.trained_samples=0; NN.mem={}; NN.best_loss=1e9
    client.color_log(180,180,255,'[ZRes] NN weights reset (v4 architecture)')
end)
...
-- ── per-player DB (v4: expanded tracking) ────────────────────────────────
local function _db(ent)
    if not _M.DB[ent] then
        _M.DB[ent]={
            ...
            lby_2nd_deriv=0,lby_feet_rate=0,lby_snap_hist={},
            ...
            jitter_freq=0,jitter_phase=0,jitter_mag=30,
            side=1,desync=35,confidence=0.5,method='none',aa_type='Gathering',aa_swing=0,
            last_nn_inp=nil,
            hit_count=0,miss_count=0,fail_streak=0,hit_streak=0,
            brute_stage=0,brute_locked=false,brute_best_off=nil,brute_attempts={},
            pitch_res=0,last_sim=0,last_tick=0,conf_last_update=0,layer_sigs={},
            avg_latency=0,shot_at_tick={},
            -- v4: per-AA-type tracking
            aa_type_hits={Jitter=0,Static=0,LBY=0,Swing=0,Sway=0,Chaotic=0},
            aa_type_misses={Jitter=0,Static=0,LBY=0,Swing=0,Sway=0,Chaotic=0},
        }
...
-- ── trackers (v4: improved jitter magnitude + LBY timing) ───────────────────
local function _track_lby(ent,d)
    ...
    -- improved LBY timing: account for network latency
    local lby_delay = 0.22 + (d.avg_latency or 0) * 0.5
    if spd>1.2 then 
        d.lby_timer=now+lby_delay; d.lby_updating=true
    elseif now>=d.lby_timer then 
        d.lby_updating=true; d.lby_timer=now+1.1
    else 
    ...
    table.insert(d.deltas,delta); if #d.deltas>150 then table.remove(d.deltas,1) end
    table.insert(d.feet_hist,feet); if #d.feet_hist>80 then table.remove(d.feet_hist,1) end
    table.insert(d.eye_hist,eye);   if #d.eye_hist>80  then table.remove(d.eye_hist,1)  end
    table.insert(d.pitch_hist,ast.m_flPitch or 0); if #d.pitch_hist>40 then table.remove(d.pitch_hist,1) end
    
    local fd=_ydelta(feet,d.lby_last_feet)
    d.lby_2nd_deriv=fd-d.lby_feet_rate; d.lby_feet_rate=fd
    if _abs(fd)>22 and spd<2.5 then d.lby_snap_cnt=d.lby_snap_cnt+1.2 end
    d.lby_snap_cnt = d.lby_snap_cnt * 0.98  -- decay
    d.lby_last_feet=feet
    
    -- track jitter magnitude (exponential moving average of delta changes)
    if #d.deltas >= 2 then
        local delta_change = _abs(d.deltas[#d.deltas] - d.deltas[#d.deltas-1])
        d.jitter_mag = (d.jitter_mag or 30) * 0.85 + delta_change * 0.15
    end
    
    table.insert(d.vel_hist,{vx=vx,vy=vy,spd=spd,t=now}); if #d.vel_hist>40 then table.remove(d.vel_hist,1) end
end
...
-- ── classifier (v4 improved with FFT jitter detection) ─────────────────────
local function _classify(d, jt, decay)
    if #d.deltas < 10 then return 'Gathering', 0, 0.5 end
    local n   = #d.deltas
    ...
    local zero_cross = 0  -- count sign crossings through zero
    local accel_sum = 0   -- track acceleration (2nd derivative)
    ...
    for i = 2, n do
        ...
        if diff < 4     then static_w = static_w + wi end
        sum_w = sum_w + c * wi
        if c > 0 then pos_w = pos_w + wi else neg_w = neg_w + wi end
        
        -- zero crossing detection (for jitter frequency)
        if (c > 5 and p < -5) or (c < -5 and p > 5) then
            zero_cross = zero_cross + 1
        end
        
        -- acceleration tracking
        if i >= 3 then
            local pp = d.deltas[i-2]
            local accel = (c - 2*p + pp)
            accel_sum = accel_sum + _abs(accel) * wi
        end
    ...
    local zc_rate = zero_cross / n  -- zero crossing rate
    local accel_rate = accel_sum / wsum
    local prev = d.aa_type
    local at

    -- store jitter frequency estimate
    d.jitter_freq = _fl(zero_cross * 64 / n)  -- normalized to ~ticks

    -- jitter: rapid flipping OR high zero-crossing rate
    if fr > 0.32 or zc_rate > 0.18 then
        at = 'Jitter'
    -- static: small deltas, consistent, low acceleration
    elseif sr > 0.65 and swing < 22 and accel_rate < 15 then
        at = 'Static'
    -- LBY: in LBY update window with moderate deltas
    elseif d.lby_updating and swing < 75 and zc_rate < 0.12 then
        at = 'LBY'
    -- swing: single large direction, sustained
    elseif swing > 75 and fr < 0.08 and max_consec > 5 then
        at = 'Swing'
    -- sway: medium oscillation, slower than jitter
    elseif swing > 18 and swing <= 75 and fr < 0.22 and zc_rate < 0.15 then
        at = 'Sway'
    -- high acceleration = chaotic
    elseif accel_rate > 40 then
        at = 'Chaotic'
    else
        at = 'Chaotic'
    end

    -- hysteresis: stronger stickiness to prevent flip-flopping
    if prev == 'Jitter' and at ~= 'Jitter' and (fr > 0.20 or zc_rate > 0.12) then at = 'Jitter' end
    if prev == 'Static' and at ~= 'Static' and sr > 0.52 then at = 'Static' end
    if prev == 'LBY'    and at ~= 'LBY'    and d.lby_updating then at = 'LBY' end
    if prev == 'Swing'  and at ~= 'Swing'  and swing > 60 and max_consec > 3 then at = 'Swing' end

    return at, swing, bias
end

-- ── jitter side predictor (v4: better period detection + phase prediction) ──
local function _jitter_predict(d)
    if #d.deltas < 8 then return d.side, 0.55 end

    -- find all sign flips with their timestamps and magnitudes
    local flips = {}
    for i = 2, #d.deltas do
        local c, p = d.deltas[i], d.deltas[i-1]
        if (c > 0) ~= (p > 0) and _abs(c) > 4 and _abs(p) > 4 then
            flips[#flips+1] = { idx=i, delta=c, mag=_abs(c) }
        end
    end

    if #flips < 3 then
        -- not enough flips: use weighted average of recent deltas
        local wsum, wtot = 0, 0
        for i = _max(1, #d.deltas-6), #d.deltas do
            local w = i / #d.deltas
            wsum = wsum + d.deltas[i] * w
            wtot = wtot + w
        end
        local avg = wtot > 0 and wsum / wtot or 0
        return avg > 0 and 2 or 1, 0.58
    end

    -- detect if jitter is symmetric (L/R same magnitude) or asymmetric
    local left_mags, right_mags = {}, {}
    for _, f in ipairs(flips) do
        if f.delta > 0 then right_mags[#right_mags+1] = f.mag
        else left_mags[#left_mags+1] = f.mag end
    end
    local avg_left = #left_mags > 0 and (function() local s=0; for _,v in ipairs(left_mags) do s=s+v end; return s/#left_mags end)() or 30
    local avg_right = #right_mags > 0 and (function() local s=0; for _,v in ipairs(right_mags) do s=s+v end; return s/#right_mags end)() or 30
    local asymmetry = _abs(avg_left - avg_right) / _max(avg_left, avg_right, 1)

    -- measure period with exponential weighting on recent flips
    local periods = {}
    for i = 2, #flips do
        periods[#periods+1] = flips[i].idx - flips[i-1].idx
    end
    
    local period_wsum, period_wtot = 0, 0
    for i, p in ipairs(periods) do
        local w = (i / #periods) ^ 1.5  -- strong recency bias
        period_wsum = period_wsum + p * w
        period_wtot = period_wtot + w
    end
    local avg_period = period_wtot > 0 and period_wsum / period_wtot or 2

    -- calculate current phase
    local last_flip = flips[#flips]
    local ticks_since = #d.deltas - last_flip.idx
    local phase = (ticks_since % _max(1, _fl(avg_period + 0.5))) / _max(1, avg_period)

    -- predict side: consider asymmetry
    local side_after_flip = last_flip.delta > 0 and 2 or 1
    local current_side
    if phase < 0.5 then
    ...
    -- if asymmetric, favor the larger side
    if asymmetry > 0.25 then
        local dominant = avg_right > avg_left and 2 or 1
        -- blend: 70% phase prediction, 30% dominant side
        if math.random() < 0.35 then current_side = dominant end
    end

    -- confidence: based on period consistency and recency of data
    local period_var = 0
    for _, p in ipairs(periods) do
        period_var = period_var + (p - avg_period)^2
    end
    period_var = #periods > 1 and period_var / (#periods - 1) or 0
    local consistency = _clamp(1 - period_var / (_max(1, avg_period)^2 + 1), 0.50, 0.94)
    
    -- reduce confidence if asymmetric (harder to predict)
    if asymmetry > 0.3 then consistency = consistency * 0.88 end

    -- store phase for debug
    d.jitter_phase = phase

    return current_side, consistency
end

-- ── LBY resolver (v4: improved snap detection + history tracking) ───────────
local function _resolve_lby(ent, d, ast)
    ...
    local goal = ast.m_flGoalFeetYaw
    local delta = _ydelta(eye, feet)
    local now = globals.curtime()

    -- Track LBY snap history for pattern recognition
    if not d.lby_snap_hist then d.lby_snap_hist = {} end
    
    -- Detect snap: rapid feet movement when standing still
    local feet_rate = _abs(d.lby_feet_rate or 0)
    if feet_rate > 20 and #d.vel_hist > 0 and d.vel_hist[#d.vel_hist].spd < 3 then
        table.insert(d.lby_snap_hist, { t=now, dir=d.lby_feet_rate > 0 and 2 or 1, feet=feet })
        if #d.lby_snap_hist > 12 then table.remove(d.lby_snap_hist, 1) end
    end

    -- If actively snapping (goal != feet), trust goal direction
    if goal and _abs(_ydelta(goal, feet)) > 8 then
        local snap_dir = _ydelta(goal, eye)
        local snap_side = snap_dir > 0 and 2 or 1
        -- confidence based on how far the snap needs to go
        local snap_dist = _abs(_ydelta(goal, feet))
        local conf = _clamp(0.88 + snap_dist * 0.002, 0.88, 0.96)
        return snap_side, 0, conf
    end

    -- LBY stable (not updating): real yaw = feet yaw
    if not d.lby_updating then
        local real_side = delta > 6 and 2 or delta < -6 and 1 or d.side
        -- desync is minimal during LBY stable
        local desync = _fl(_abs(delta) * 0.25)
        return real_side, desync, 0.92
    end

    -- LBY is updating: predict snap direction
    -- Use recent snap history if available
    if #d.lby_snap_hist >= 2 then
        local cnt = {[1]=0, [2]=0}
        for i = _max(1, #d.lby_snap_hist - 4), #d.lby_snap_hist do
            local s = d.lby_snap_hist[i]
            if s then cnt[s.dir] = cnt[s.dir] + 1 end
        end
        local dom_side = cnt[2] >= cnt[1] and 2 or 1
        return dom_side, 12, 0.85
    end

    -- fallback: use 2nd derivative to predict
    local snap_dir = d.lby_2nd_deriv > 0 and 2 or 1
    return snap_dir, 18, 0.78
end
...
-- ── main pattern resolver (v4: improved desync estimation) ─────────────────
local function _pattern(ent, d, state, ld, jt, decay)
    ...
    if at == 'Static' then
        ...
        if ps then
            side   = ps
            desync = _clamp(_fl(pd2 * 0.88), 32, 60)
            conf   = 0.93
        else
            -- fall back to last delta sign with better estimation
            side   = rd > 0 and 2 or 1
            -- use max of recent deltas for better desync estimate
            local max_delta = 0
            for i = _max(1, #d.deltas - 5), #d.deltas do
                max_delta = _max(max_delta, _abs(d.deltas[i] or 0))
            end
            desync = _clamp(_fl(_max(_abs(rd), max_delta) * 0.82), 25, 60)
            conf   = 0.89
        end
        d.method = 'static'
    ...
    elseif at == 'Jitter' then
        ...
        -- use jitter magnitude for desync, not just current delta
        local jitter_mag = d.jitter_mag or swing
        desync = _clamp(_fl(_max(_abs(rd), jitter_mag) * 0.72), 18, 55)
        if desync < 15 then desync = _max(20, _fl(swing * 0.68)) end
        d.method = 'jitter'
    ...
    elseif at == 'Swing' then
        ...
        side   = rd > 0 and 2 or 1
        desync = _clamp(_fl(_abs(rd) * 0.90), 38, 60)
        conf   = 0.76
        d.method = 'swing'

    elseif at == 'Sway' then
        -- exponentially weighted average direction with velocity consideration
        local w = _exp_weights(#d.deltas, decay)
        local ws, wt = 0, 0
        local max_d = 0
        for i, v in ipairs(d.deltas) do 
            ws = ws + v * w[i]; wt = wt + w[i]
            max_d = _max(max_d, _abs(v))
        end
        local wavg = wt > 0 and ws / wt or 0
        side   = wavg > 0 and 2 or 1
        -- blend current delta with historical max
        desync = _clamp(_fl((_abs(rd) * 0.6 + max_d * 0.4) * 0.75), 20, 52)
        conf   = 0.68
        d.method = 'sway'

    else  -- Chaotic / Gathering
        -- use current real delta + history for chaotic
        local hist_avg = 0
        if #d.deltas > 3 then
            for i = _max(1, #d.deltas - 4), #d.deltas do
                hist_avg = hist_avg + _abs(d.deltas[i] or 0)
            end
            hist_avg = hist_avg / _min(5, #d.deltas)
        end
        side   = rd > 4 and 2 or rd < -4 and 1 or d.side
        desync = _clamp(_fl(_max(_abs(rd), hist_avg) * 0.65), 12, 42)
        conf   = 0.45
        d.method = 'chaotic'
    end
    ...
    if ld.lean_w > 0.28 and d.lean_side ~= 0 then
        local ls = d.lean_side > 0 and 2 or 1
        local lc = _clamp(d.lean_magnitude * 1.7, 0.58, 0.88)
        if lc > conf then
            side = ls; desync = _fl(ld.lean_w * 42); conf = lc
            d.method = d.method .. '+lean'
        end
    end

    -- ── desync cycle correction (animation layer desync signal) ─────────
    if ld.desync_cycle > 0.15 and ld.desync_cycle < 0.85 then
        -- desync cycle indicates real desync amount
        local dc_desync = _fl(ld.desync_cycle * 70)
        desync = _fl((desync * 0.6) + (dc_desync * 0.4))
        d.method = d.method .. '+dc'
    end

    -- ── defensive override ──────────────────────────────────────────────
    if d.def_active and _M.ui_exploits:get() then
        side   = d.def_side
        desync = _max(desync, d.def_snap_cnt > d.def_flick_cnt and 52 or 56)
        conf   = _max(conf, 0.82)
    ...
    if d.dt_active and _M.ui_exploits:get() then
        desync = _max(desync, 54); conf = _max(conf, 0.76)
        d.method = d.method .. '+dt'
    end
    if d.fd_active and _M.ui_exploits:get() then
        desync = _min(desync, 16); conf = _max(conf, 0.72)
...
-- ── brute force (v4: smarter ordering + side priority + hit-based locking) ──
local _BRUTE_SMART = {58,-58,50,-50,42,-42,34,-34,26,-26,18,-18,10,-10,0}
local _BRUTE_FAST  = {58,-58,0,42,-42,22,-22}
local _BRUTE_FULL  = {60,-60,55,-55,50,-50,45,-45,40,-40,35,-35,30,-30,25,-25,20,-20,15,-15,10,-10,5,-5,0}
local _BRUTE = _BRUTE_SMART

local function _brute(d)
    -- locked winning offset takes priority
    ...
    -- select brute table based on AA type
    if d.aa_type == 'Static' then
        _BRUTE = _BRUTE_SMART  -- static usually has max desync
    elseif d.aa_type == 'Jitter' then
        _BRUTE = _BRUTE_FULL   -- jitter needs more precision
    else
        _BRUTE = _BRUTE_SMART
    end

    -- prioritize side based on hit history
    local side_bias = 0  -- 0 = no bias, 1 = left, 2 = right
    if d.side_hits then
        local sh = d.side_hits
        if sh[2] > sh[1] * 1.5 then side_bias = 2
        elseif sh[1] > sh[2] * 1.5 then side_bias = 1 end
    end
    ...
    if d.adaptive_desync and d.fail_streak >= 3 then
        ...
        for _, bv in ipairs(_BRUTE) do
            ...
            -- apply side bias: prefer values matching the bias
            if side_bias > 0 then
                local bv_side = bv >= 0 and 2 or 1
                if bv_side == side_bias then dist = dist * 0.7 end
            end
        ...
        -- apply side bias to normal brute
        if side_bias > 0 and d.fail_streak >= 2 then
            local sign = side_bias == 2 and 1 or -1
            v = sign * _abs(v)
        end
        d.method = _f('brute@%+d', v)
    end

    -- track brute attempts for analysis
    if not d.brute_attempts then d.brute_attempts = {} end
    table.insert(d.brute_attempts, v)
    if #d.brute_attempts > 30 then table.remove(d.brute_attempts, 1) end

    return v >= 0 and 2 or 1, _abs(v), 1.0
end

-- ── NN forward pass (v4: larger network, dropout, better activation) ────────
local function _nn_fwd(inp, training)
    local NN = _M.NN3
    local h  = {}
    local dropout = training and NN.cfg.dropout or 0
    
    for i = 1, 64 do
        local s = 0
        for j = 1, 24 do s = s + (inp[j] or 0) * NN.wh[j][i] end
        -- leaky relu with steeper negative slope
        h[i] = s > 0 and s or 0.015 * s
        -- dropout during training
        if dropout > 0 and math.random() < dropout then h[i] = 0 end
    end
    local o = {}
    for i = 1, 4 do
        local s = 0
        for j = 1, 64 do s = s + h[j] * NN.wo[j][i] end
        o[i] = 1 / (1 + _exp(-_clamp(s, -15, 15)))
    end
    return o, h
end
...
local function _nn_inp(ent, d, state, ld)
    ...
    local d4   = d.deltas[#d.deltas-3]   or 0
    
    -- calculate delta velocity (acceleration of yaw changes)
    local delta_vel = #d.deltas >= 2 and (d1 - d2) or 0
    ...
    return {
        ...
        _clamp(d.jitter_freq/64, 0, 1),
        -- exploit signals (3)
        d.def_active and 1 or 0,
        d.dt_active  and 1 or 0,
        d.fd_active  and 1 or 0,
    }
end
...
local function _hybrid(ent, d, state, ld, jt, decay)
    ...
    local nout, _ = _nn_fwd(inp, false)  -- inference mode (no dropout)
    local ns = nout[1] > 0.52 and 2 or 1
    local nd = _fl(nout[2] * 62)
    local nc = nout[3]
    local n_aa_type = nout[4]  -- 4th output: AA type confidence
    d.last_nn_inp = inp

    local trust = _clamp(((_M.NN3.trained_samples or 0)) / 20, 0, 1)

    -- always trust pattern for LBY or very high pattern confidence
    if d.lby_updating or pc > 0.90 or trust < 0.20 then
        return ps, pd, pc
    end

    -- NN takes over for jitter/chaotic when well trained
    if (d.aa_type == 'Jitter' or d.aa_type == 'Chaotic') and trust > 0.45 then
        local eff_nc = nc * trust
        if eff_nc > pc * 0.9 then
            d.method = 'neural+' .. d.method
            return ns, nd, _min(eff_nc, 0.94)
        end
    end

    -- use NN for static when it disagrees with pattern
    if d.aa_type == 'Static' and trust > 0.6 and ns ~= ps then
        d.method = 'neural_static+' .. d.method
        return ns, nd, nc * trust
    end

    -- weighted blend with trust-based scaling
    local pw = pc; local nw = nc * trust * 0.85; local tot = pw + nw
...
-- ── NN training (v4: learning rate schedule, better backprop) ─────────────
local function _nn_train()
    local NN = _M.NN3
    if #NN.mem < 32 then return end
    
    -- learning rate schedule
    local lr = NN.cfg.lr
    for threshold, new_lr in pairs(NN.lr_schedule) do
        if NN.trained_samples >= threshold then lr = new_lr end
    end
    
    for _ = 1, 16 do
        local s = NN.mem[math.random(1, #NN.mem)]
        local o, h = _nn_fwd(s.inp, true)  -- training mode (with dropout)
        local sw = s.w or 1
        
        -- output deltas (4 outputs now)
        local do4 = {}
        for i = 1, 4 do
            do4[i] = (s.tgt[i] - o[i]) * o[i] * (1 - o[i]) * sw
        end
        
        -- output weights with gradient clipping
        for i = 1, 4 do
            for j = 1, 64 do
                local g = lr * do4[i] * h[j]
                g = _clamp(g, -0.5, 0.5)  -- gradient clipping
        ...
        for j = 1, 64 do
            local dh = 0
            for i = 1, 4 do dh = dh + do4[i] * NN.wo[j][i] end
            dh = dh * (h[j] > 0 and 1 or 0.015)
            for k = 1, 24 do
                local g = lr * dh * (s.inp[k] or 0)
                g = _clamp(g, -0.5, 0.5)  -- gradient clipping
...
local function _nn_add(d, hit, hs)
    ...
    -- weight: headshots 4x, body hits 1.5x, misses 2x (learn from mistakes)
    local w = hs and 4.0 or hit and 1.5 or 2.0
    
    -- AA type encoding for 4th output
    local aa_type_val = 0.5
    if d.aa_type == 'Jitter' then aa_type_val = 0.9
    elseif d.aa_type == 'Static' then aa_type_val = 0.1
    elseif d.aa_type == 'LBY' then aa_type_val = 0.3
    elseif d.aa_type == 'Swing' then aa_type_val = 0.7 end
    
    table.insert(NN.mem, {
        inp = d.last_nn_inp,
        tgt = { 
            d.side==2 and 1.0 or 0.0,   -- side
            _min(d.desync/62,1),         -- desync
            hit and 1.0 or 0.0,          -- hit confidence
            aa_type_val                  -- AA type
        },
        w   = w
    })
    if #NN.mem > 2500 then table.remove(NN.mem, 1) end
end
...
client.set_event_callback('paint',function()
    ...
    for _,ent in ipairs(enemies) do
        ...
        local side,desync,conf
        -- auto brute threshold (v4: dynamic based on AA type + confidence)
        local _auto_bt = d.aa_type=="Jitter" and 3
            or d.aa_type=="Chaotic" and 2
            or d.aa_type=="Static" and 5
            or d.aa_type=="LBY" and 4
            or d.aa_type=="Swing" and 4
            or d.aa_type=="Sway" and 3
            or 4
        -- if confidence is low, lower brute threshold
        if d.confidence < 0.5 then _auto_bt = _max(2, _auto_bt - 1) end
        -- check if we should brute
        local should_brute = d.fail_streak >= _auto_bt or mode=='Bruteforce Only'
        if should_brute then side,desync,conf=_brute(d)
...
-- ── aim_hit (v4: AA type tracking + confidence boost) ────────────────────
client.set_event_callback('aim_hit',function(e)
    if not _auth_alive or not _M.ui_en:get() then return end
    ...
    -- update per-AA-type stats
    local at = d.aa_type or 'Chaotic'
    if d.aa_type_hits[at] then d.aa_type_hits[at] = d.aa_type_hits[at] + (hs and 2 or 1) end
    
    if hs then 
        d.fail_streak=0; d.brute_stage=0; d.brute_locked=false; d.miss_pattern={}
        -- boost confidence on headshot
        d.confidence = _min(1.0, d.confidence + 0.12)
    else 
        d.fail_streak=_max(0,d.fail_streak-1)
        if d.brute_stage>0 then d.brute_stage=_max(0,d.brute_stage-1) end
        d.confidence = _min(1.0, d.confidence + 0.04)
    end
    ...
    -- NN training on hit (always train, not just hybrid mode)
    _nn_add(d, true, hs)
...
client.set_event_callback('aim_miss',function(e)
    ...
    -- update per-AA-type stats
    local at = d.aa_type or 'Chaotic'
    if d.aa_type_misses[at] then d.aa_type_misses[at] = d.aa_type_misses[at] + 1 end
    
    d.miss_count=d.miss_count+1; d.fail_streak=d.fail_streak+1; d.hit_streak=0
    d.brute_stage=(d.brute_stage+1)%#_BRUTE
    
    -- confidence penalty on miss
    d.confidence = _max(0.2, d.confidence - 0.08)
    
    -- smarter side flip: use AA type history if available
    local should_flip = true
    if d.aa_type_hits[at] and d.aa_type_misses[at] then
        local total = d.aa_type_hits[at] + d.aa_type_misses[at]
        if total >= 3 then
            local hit_rate = d.aa_type_hits[at] / total
            -- if we're doing well with current side on this AA type, don't flip immediately
            if hit_rate > 0.55 and d.fail_streak < 2 then should_flip = false end
        end
    end
    
    if should_flip then d.side = d.side==1 and 2 or 1 end
    ...
    -- NN training on miss (always train)
    _nn_add(d, false, false)
...
-- ── round_start ──────────────────────────────────────────────────────────
-- NN weight persistence (v4: larger architecture)
local _NN_DB_KEY='zenith_nn_weights_v4'
local function _nn_save()
    if not _M.ui_nn_save or not _M.ui_nn_save:get() then return end
    local NN=_M.NN3
    pcall(database.write,_NN_DB_KEY,{
        wh=NN.wh,wo=NN.wo,
        trained=NN.trained_samples or 0,
        best_loss=NN.best_loss or 1e9,
        version=4
    })
end
local function _nn_load()
    ...
    if ok and type(v)=='table' and v.wh and v.wo and v.version==4 then
        _M.NN3.wh=v.wh; _M.NN3.wo=v.wo
        _M.NN3.trained_samples=v.trained or 0
        _M.NN3.best_loss=v.best_loss or 1e9
        client.color_log(100,255,150,'[ZRes] NN v4 weights loaded ('..tostring(v.trained or 0)..' samples)')
    elseif ok and type(v)=='table' and v.wh and not v.version then
        -- old v3 weights, skip and re-init
        client.color_log(255,200,100,'[ZRes] Old NN weights found, re-initializing for v4 architecture')
    end
end
